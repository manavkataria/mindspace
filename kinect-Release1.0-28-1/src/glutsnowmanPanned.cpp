...
{
	switch (key) {
		case GLUT_KEY_LEFT : moveMePan(1);break;
		//temp: case GLUT_KEY_LEFT : angle -= 0.01f;orientMe(angle);break; //rotation
		case GLUT_KEY_RIGHT : moveMePan(-1);break;			//panning right; hitch/needs orient me 
															//to be called before this works.
		//case GLUT_KEY_RIGHT : angle +=0.01f;orientMe(angle);break;
		
		case GLUT_KEY_UP : moveMeFlat(1);break;			//Zoom In
		case GLUT_KEY_DOWN : moveMeFlat(-1);break;		//Zoom Out
	}
}

extern void parentIdleFunction();
int whSnowman;
